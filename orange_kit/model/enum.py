...
class EnumMeta(type):
  ...
  def __new__(mcs, name, bases, class_dict, **kwargs):
    ...
    name_dict = {}
    value_dict = {}
    class_dict['__name_dict__'] = name_dict
    class_dict['__value_dict__'] = value_dict
    enum_class = super().__new__(mcs, name, bases, class_dict, **kwargs)
    for k, v in class_dict['__enum_raw_item_dict__'].items():
      value = v[0]
      enum = enum_class(k, value, v[1])
      name_dict[k] = enum
      value_dict[value] = enum
      setattr(enum_class, k, enum)
    enum_dict[class_dict.get('__module__')+"."+name] = enum_class
    return enum_class


class BaseEnum(metaclass=EnumMeta):

  __slots__ = ("name","value","desc",'value_type')
